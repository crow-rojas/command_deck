...
export class Overlay {
  ...
  init() {
    ...
    this.resultWrap = el('div', { id: 'cd-result-wrap' });
    const resultToolbar = el('div', { class: 'cd-result-toolbar' });
    this.copyBtn = el('button', { class: 'cd-icon-btn', id: 'cd-copy-result', title: 'Copy result' }, ['📋']);
    this.toggleResultBtn = el('button', { class: 'cd-icon-btn', id: 'cd-toggle-result', title: 'Hide results' }, ['▾']);
    resultToolbar.appendChild(this.copyBtn);
    resultToolbar.appendChild(this.toggleResultBtn);
    this.resultPre = el('pre', { id: 'command-deck-output' });
    this.resultWrap.appendChild(resultToolbar);
    this.resultWrap.appendChild(this.resultPre);

    this.panel.appendChild(headerWrap);
    this.panel.appendChild(this.tabsWrap);
    this.panel.appendChild(this.actionsWrap);
    this.panel.appendChild(this.resultWrap);
    ...
    this.toggleResultBtn.addEventListener('click', () => {
      const visible = this.resultPre.style.display === 'none' ? false : true;
      this.setResultsVisible(!visible);
    });

    this.copyBtn.addEventListener('click', async () => {
      const text = this.resultPre.textContent || '';
      try {
    ...
    // Initial result visibility (persisted)
    const initialVis = store.get('results-visible', '1') !== '0';
    this.setResultsVisible(initialVis);
  }
  ...
  setResultsVisible(show) {
    if (!this.resultPre) return;
    this.resultPre.style.display = show ? 'block' : 'none';
    if (this.toggleResultBtn) {
      this.toggleResultBtn.textContent = show ? '▾' : '▸';
      this.toggleResultBtn.setAttribute('title', show ? 'Hide results' : 'Show results');
    }
  ...
  setRunning(){
    ...
    this.resultPre.textContent = 'Running...';
  }
  setResult(obj){
    this.setResultsVisible(true);
    this.resultPre.textContent = jsonPretty(obj);
  }
}
