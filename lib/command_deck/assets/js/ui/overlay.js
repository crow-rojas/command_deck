...
export class Overlay {
  ...
  init() {
    ...
    // Result view is self-contained; it initializes its own state
  }
  ...
  renderActions(tab) {
    ...
    (tab.actions || []).forEach(action => {
      ...
      run.addEventListener('click', () => {
        ...
        this.resultView.setRunning();
        this.actionsApi
          .submit(action.key, payload)
          .then((res) => { this.resultView.setResult(res); this.refreshCurrentTab(); })
          .catch((e) => this.resultView.setResult({ ok:false, error: String(e) }))
  ...
  // Result handling delegated to ResultView
}
